...
@Version : 1.2
*/
class PencarianGoogle extends Actions
{
	var $keyword	=	"testing";
	var $urlList	=	array();
	var $time1		=	4000000;
	...
	function initGoogle() {
		$data=$this->getpagedata('http://www.google.co.id');		//	Open google.com ( Might redirect to country specific site e.g. www.google.co.in)
		$this->pause();
	...
	public static function google($channel, $nickname, $args) {
	ini_set ( 'max_execution_time', 0);
	
    if(!($args)) {
    return "Pencyaryan Google By Syandal_ .. argumen harus dimasukin .. "; }
    else {
 
	$stringargumen = implode(" ", $args);
	$hasilgoogle = new PencarianGoogle();
	$hasilarray = $hasilgoogle->getUrlList(urlencode($stringargumen), '');
